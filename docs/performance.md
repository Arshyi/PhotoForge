...
Version 0.7.1 extends the reproducible release benchmark across 1920×1080, 4000×3000, and 6000×4000 fixtures. It includes rectangle, freehand, a 500-point polygon, full-image Magic Wand, multiple feather radii, morphology, refinement, thumbnail generation, crop/quarter-turn/straighten/perspective/lens-distortion remaps, JSON and PNG interchange, masked brightness and Decontaminate Colors processing, masked export, cancellation acknowledgement, and owned-buffer observations. Confirmed release figures are recorded in [phase-7.1-results.md](phase-7.1-results.md); historical 0.7.0 figures above remain unchanged.

The 0.7.1 thumbnail cache retains at most 96 rendered thumbnails and approximately 2 MiB. The output uses area averaging and never recalculates the image-processing pipeline. Geometry batches cap their item count and aggregate pixel allocation, and their general transform path bilinearly samples destination coverage once; the lens inverse is iteration-bounded and rejects folded or near-singular distortion. The numerical progress UI is delayed 180 ms so short operations do not add visual churn. JSON/PNG mask interchange reports real byte, row, and pixel units where available, labels parser/codec-only phases without invented local units, and checks cancellation between bounded chunks and before destination replacement.
