...
Later Phase 7.1 work resolves these 0.7.0 limitations with real thumbnails and visible named-mask overlays; transactional remapping for crop, exact rotation, horizontal reflection, straighten, perspective, and bounded lens distortion; a dedicated Refine dialog with optional masked-only Decontaminate Colors; work-unit progress extended through JSON/PNG mask interchange; optional resolved pen pressure; strict full-resolution mask-stage validation; and selection-session schema 2 with Phase 7 migration. These are later resolutions and do not change the historical 0.7.0 measurements or claims above. Packaged lifecycle, manual-matrix, performance, network, test, and artifact results for the hardening release are reported separately in [phase-7.1-results.md](phase-7.1-results.md).
