...
This repository contains the Phase 0 foundation, Phase 1 editor, Phase 1.1 hardening, Phase 2 deterministic restoration tools, and Phase 3 guided local editing. The current version is **0.3.0**.
...
PhotoForge Phase 3 uses deterministic rule matching to propose existing deterministic image operations. The planner does not edit pixels, execute code, contact a model, or bypass plan validation. It does not generate missing image content or reconstruct factual details that were never captured. Ollama, neural restoration, OCR, perspective correction, and generative editing are intentionally outside this release.
